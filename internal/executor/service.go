...
func normalizedFeatures(supplied domain.RequestFeatures, request inference.Request, turn conversation.Turn) domain.RequestFeatures {
	...
	for _, item := range items {
		...
		if strings.HasSuffix(item.Type, "_call_output") || item.Type == "function_call" ||
			item.Type == "custom_tool_call" || item.Type == "additional_tools" {
			features.NeedsFunctions = true
		}
		if (strings.HasSuffix(item.Type, "_call_output") && item.Type != "function_call_output") ||
			item.Type == "custom_tool_call" || item.Type == "additional_tools" {
